...
// A live region must already exist in the DOM for a text change to be announced, so
// the node below is permanent and never toggled with v-if.
...
<template>
  <div class="flex flex-col gap-1.5">
    ...
    <!-- Card radius, not the control radius other inputs use, and `rule-dashed` —
         a darker neutral than both `rule` and `rule-strong`. -->
    <div
      v-if="!modelValue"
    ...
    <div v-else class="rounded-control border border-rule bg-card p-3 flex items-center gap-3">
      <!-- Dedicated file-* tokens, kept independent so a rejected-* retint does not
           follow here. -->
